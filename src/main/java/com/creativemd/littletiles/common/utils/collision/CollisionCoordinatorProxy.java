...
public class CollisionCoordinatorProxy {
    ...
    private final org.joml.Vector3d rotationCenter;
    public final IVecOriginProxy moving;
    ...
    public CollisionCoordinatorProxy(double offX, double offY, double offZ, double rotX, double rotY, double rotZ, IVecOriginProxy moving, IVecOriginProxy origin) {
        this.offX = offX;
        this.offY = offY;
        this.offZ = offZ;
        this.rotX = rotX;
        ...
        this.hasOneRotation = BooleanUtilsProxy.oneTrue(hasRotX, hasRotY, hasRotZ);
        ...
        this.rotationX = hasRotX ? MatrixUtilsProxy.createRotationMatrixX(rotX) : null;
        this.rotationY = hasRotY ? MatrixUtilsProxy.createRotationMatrixY(rotY) : null;
        this.rotationZ = hasRotZ ? MatrixUtilsProxy.createRotationMatrixZ(rotZ) : null;
    ...
    public void reset(IVecOriginProxy origin) {
        this.origin = origin;
    ...
    public double getRotationDegree(EnumFacingProxy.Axis axis) {
        switch (axis) {
        case X:
            return rotX;
        case Y:
    ...
    public AxisAlignedBB computeSurroundingBox(AxisAlignedBB box) {
        return BoxUtilsProxy.getRotatedSurrounding(box, this);
    }

    private boolean isSimple() {
        if (BooleanUtilsProxy.oneTrue(hasRotX, hasRotY, hasRotZ, hasTranslation)) {
            if (hasRotX)
    ...
    public Matrix4d getInverted(Double delta) {
        ...
        if (matrix != null) {
            ...
            matrix = MatrixUtilsProxy.createRotationMatrixAndTranslationRadians(-offX * delta, -offY * delta, -offZ * delta, -rotXRadians * delta, -rotYRadians * delta, -rotZRadians * delta);
