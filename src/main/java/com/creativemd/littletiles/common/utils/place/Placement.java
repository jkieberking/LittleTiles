...
//    public final LittleIngredients removedIngredients;
    ...
    public final List<Block.SoundType> soundsToBePlayed = new ArrayList<>();
...
//    }
    ...
    public boolean canPlace() throws LittleActionException {
        ...
        for (BlockPos pos : blocks.keySet()) {
            if (!LittleAction.isAllowedToInteract(player, true)) {
...
//                    for (BlockPos pos : blocks.keySet())
//                        LittleAction.sendBlockResetToClient(world, player, pos);
//                    throw new LittleTilesConfig.NotAllowedToPlaceException(player);
//                }

                // @TODO add transparency restricted
...
//
//             @TODO multi place event, not sure what this is
...
//            }
        ...
        try {
            ...
            // @TODO block reset if something goes wrong
//            for (BlockPos snapPos : blocks.keySet())
//                LittleAction.sendBlockResetToClient(world, player, pos);
            throw e;
        }
...
//        result.parentStructure = origin.isStructure() ? origin.getStructure() : null;
        ...
        LittleNeighborUpdateCollectorProxy neighbor = new LittleNeighborUpdateCollectorProxy(world, blocks.keySet());
...
//    }
    ...
    private void createPreviews(PlacementStructurePreview current, LittleVec inBlockOffset, BlockPos pos) {
        if (current.previews != null) {
            HashMapListProxy<BlockPos, PlacePreview> splitted = new HashMapListProxy<>();
            for (PlacePreview pp : current.previews.getPlacePreviews(inBlockOffset)) {
                ...
                // @TODO unplaceabletiles !important
//                if (volume.has())
//                    unplaceableTiles.addPreview(pos, volume.createFakePreview(pp.preview), current.previews.getContext());
    ...
    public class PlacementBlock implements IGridBased {
        ...
        public final BlockPos pos;
        private TileEntityLittleTilesProxy cached;
        ...
        public PlacementBlock(BlockPos pos, LittleGridContext context) {
            ...
            if (tileEntity instanceof TileEntityLittleTilesProxy) {
                cached = (TileEntityLittleTilesProxy) tileEntity;
...
//                attribute |= structure.getAttribute();
        ...
        public boolean canPlace() throws LittleActionException {
            ...
            if (!ignoreWorldBoundaries && (pos.getY() < 0 || pos.getY() >= 256))
                return false;
            // @TODO load te
//            TileEntityLittleTilesProxy te = LittleAction.loadTe(player, world, pos, null, false, attribute);
//            if (te != null) {
//
...
//                throw new LittleTilesConfig.TooDenseException();
            ...
            if (block.getMaterial().isReplaceable())
                return true;
//            @TODO commenting this out might cause issues
                    ...
                    try {
                        ...
                        cached.updateTilesSecretly((x) -> {
                            ...
                            for (int i = 0; i < previews.length; i++) {
                                ...
                                ParentTileList parent = (ParentTileList) x.noneStructureTiles();
...
//                                }
                                ...
                                mode.prepareBlock(Placement.this, this, parent, collsionTest);
...
//                    preview.placeTile(Placement.this/*, parent*//*, structure.getStructure()*/, false);
            }
        }
