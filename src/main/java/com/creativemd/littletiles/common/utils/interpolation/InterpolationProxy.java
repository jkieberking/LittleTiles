...
import com.creativemd.littletiles.common.utils.math.vec.Vec1Proxy;
import com.creativemd.littletiles.common.utils.math.vec.VecProxy;

public abstract class InterpolationProxy<T extends VecProxy> {
    ...
    public InterpolationProxy(double[] times, T[] points) {
        if (points.length < 2)
    ...
    public InterpolationProxy(T... points) {
        if (points.length < 2)
            throw new IllegalArgumentException("At least two points are needed!");
    ...
    public T valueAt(double t) {
        if (t >= 0 && t <= 1) {
            ...
            T vec = (T) VecProxy.copyVec(firstPoint.getValue());
        ...
        return (T) Vec1Proxy.createEmptyVec(classOfT);
    }
