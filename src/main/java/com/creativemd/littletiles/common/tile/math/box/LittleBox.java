...
//    // ================Conversions================
    ...
    public void addCollisionBoxes(LittleGridContext context, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, BlockPos offset) {
        ...
        if (bBoxintersects(entityBox, axisalignedbb))
            collidingBoxes.add(axisalignedbb);
    }

    public boolean bBoxintersects(AxisAlignedBB main, AxisAlignedBB other)
    {
        return bBoxIntersects(main, other.minX, other.minY, other.minZ, other.maxX, other.maxY, other.maxZ);
    }

    public boolean bBoxIntersects(AxisAlignedBB main, double x1, double y1, double z1, double x2, double y2, double z2)
    {
        return main.minX < x2 && main.maxX > x1 && main.minY < y2 && main.maxY > y1 && main.minZ < z2 && main.maxZ > z1;
    }
    ...
    public AxisAlignedBB getBox(LittleGridContext context, BlockPos offset) {
        return AxisAlignedBB.getBoundingBox(context.toVanillaGrid(minX) + offset.getX(), context.toVanillaGrid(minY) + offset.getY(), context.toVanillaGrid(minZ) + offset.getZ(), context
    ...
    public AxisAlignedBB getBox(LittleGridContext context) {
        return AxisAlignedBB.getBoundingBox(context.toVanillaGrid(minX), context.toVanillaGrid(minY), context.toVanillaGrid(minZ), context.toVanillaGrid(maxX), context
...
//
    ...
    public int getSize(EnumFacingProxy.Axis axis) {
        switch (axis) {
            case X:
                return maxX - minX;
            case Y:
    ...
    public void setMin(EnumFacingProxy.Axis axis, int value) {
        switch (axis) {
            case X:
                minX = value;
                break;
    ...
    public int getMin(EnumFacingProxy.Axis axis) {
        switch (axis) {
            case X:
                return minX;
            case Y:
    ...
    public void setMax(EnumFacingProxy.Axis axis, int value) {
        switch (axis) {
            case X:
                maxX = value;
                break;
    ...
    public int getMax(EnumFacingProxy.Axis axis) {
        switch (axis) {
            case X:
                return maxX;
            case Y:
...
//
    ...
    public void split(LittleGridContext context, BlockPos offset, HashMapListProxy<BlockPos, LittleBox> boxes, @Nullable LittleBoxReturnedVolume volume) {
    ...
    public SplitRangeBoxes split(List<LittleBox> boxes) {
        RangedBitSetProxy x = split(Axis.Xaxis, boxes);
        RangedBitSetProxy y = split(Axis.Yaxis, boxes);
        RangedBitSetProxy z = split(Axis.Zaxis, boxes);
        if (x != null && y != null && z != null)
            return new SplitRangeBoxes(x, y, z);
        return null;
    }

    protected RangedBitSetProxy split(Axis axis, List<LittleBox> boxes) {
        int min = getMin(axis);
        int max = getMax(axis);
        RangedBitSetProxy set = new RangedBitSetProxy(min, max);
    ...
    public List<LittleBox> cutOut(List<LittleBox> boxes, List<LittleBox> cutout/*, @Nullable LittleBoxReturnedVolume volume*/) {
        ...
        if ((ranges = split(boxes)) != null) {
            ...
            boolean[][][] filled = new boolean[getSize(Axis.Xaxis)][getSize(Axis.Yaxis)][getSize(Axis.Zaxis)];
...
//                                boxes.add(extractBox(x, y, z, volume));
    ...
    public static boolean intersectsWith(LittleBox box1, LittleBox box2) {
        return box1.maxX > box2.minX && box1.minX < box2.maxX && box1.maxY > box2.minY && box1.minY < box2.maxY && box1.maxZ > box2.minZ && box1.minZ < box2.maxZ;
    }
...
//    // ================Vectors================
    ...
    public boolean intersectsWithFace(EnumFacingProxy facing, LittleVec vec) {
        EnumFacingProxy.Axis one = RotationUtilsProxy.getOne(facing.getAxis());
        EnumFacingProxy.Axis two = RotationUtilsProxy.getOne(facing.getAxis());
        return vec.get(one) >= getMin(one) && vec.get(one) <= getMax(one) && vec.get(two) >= getMin(two) && vec.get(two) <= getMax(two);
    }

    public boolean intersectsWithAxis(LittleGridContext context, EnumFacingProxy.Axis axis, Vector3d vec) {
        switch (axis) {
            case X:
    ...
    public boolean intersectsWithYZ(LittleGridContext context, Vector3d vec) {
    ...
    @Nullable
    protected Vector3d collideWithPlane(LittleGridContext context, EnumFacingProxy.Axis axis, double value, Vector3d vecA, Vector3d vecB) {
        Vector3d vec3d = axis != EnumFacingProxy.Axis.X ? axis !=  EnumFacingProxy.Axis.Y ? getIntermediateWithZValue(vecA, vecB, value) : getIntermediateWithYValue(vecA, vecB, value) :
            getIntermediateWithXValue(vecA, vecB, value);
    ...
    public MovingObjectPosition calculateIntercept(LittleGridContext context, BlockPos pos, Vector3d vecA, Vector3d vecB) {
        vecA = vecA.sub(new Vector3d(pos.getX(), pos.getY(), pos.getZ()));
        ...
        if (collision == null)
            return null;
        Vector3d addedVector = addVector(collision, pos.getX(), pos.getY(), pos.getZ());
        // @TODO this might be wrong, since going from vector double to int's
        return new MovingObjectPosition(((int) Math.floor(addedVector.x)), (int) Math.floor(addedVector.y), (int) Math.floor(addedVector.z), /* @TODO .ordinal() might be wrong */ collided.ordinal(), Vec3.createVectorHelper(pos.x, pos.y, pos.z));
    }

    public static Vector3d addVector(Vector3d vector, double x, double y, double z)
    {
...
//    // ================Special methods================
    ...
    public boolean isFaceAtEdge(LittleGridContext context, EnumFacingProxy facing) {
        if (facing.getAxisDirection() == EnumFacingProxy.AxisDirection.POSITIVE)
...
//    }
    ...
    public static boolean isClosest(Vector3d from, @Nullable Vector3d optional, Vector3d toCheck) {
        return optional == null || squareDistanceTo(from, toCheck) < squareDistanceTo(from, optional);
    }

    // this is from 1.12's Vector3d class
    ...
    // this is from 1.12's Vector3d class
    @Nullable
    public Vector3d getIntermediateWithXValue(Vector3d vec1, Vector3d vec2, double x)
    {
    ...
    // this is from 1.12's Vector3d class
    @Nullable
    public Vector3d getIntermediateWithYValue(Vector3d vec1, Vector3d vec2, double y)
    {
        double d0 = vec2.x - vec1.x;
        double d1 = vec2.y - vec1.y;
        double d2 = vec2.z - vec1.z;
    ...
    // this is from 1.12's Vector3d class
    @Nullable
    public Vector3d getIntermediateWithZValue(Vector3d vec1, Vector3d vec2, double z)
    {
        double d0 = vec2.x - vec1.x;
        double d1 = vec2.y - vec1.y;
        double d2 = vec2.z - vec1.z;
