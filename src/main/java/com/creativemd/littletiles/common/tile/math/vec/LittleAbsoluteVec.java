...
public class LittleAbsoluteVec implements IGridBased {
    ...
    public LittleAbsoluteVec(MovingObjectPosition movingObjectPosition, LittleGridContext context) {
        long x = context.toGridAccurate(movingObjectPosition.hitVec.xCoord);
        long y = context.toGridAccurate(movingObjectPosition.hitVec.yCoord);
        long z = context.toGridAccurate(movingObjectPosition.hitVec.zCoord);
...
//
    ...
    public LittleVec getRelative(BlockPos pos) {
        LittleVec newVec = new LittleVec(getContext(), LittleUtils.subtractVectors(this.pos, pos));
    ...
    public LittleVecContext getRelative(LittleAbsoluteVec pos) {
        forceContext(pos);
        LittleVecContext newVec = new LittleVecContext(new LittleVec(getContext(), LittleUtils.subtractVectors(this.pos, pos.pos)), getContext());
    ...
    public void add(LittleAbsoluteVec pos) {
        this.pos = LittleUtils.addVectors(this.pos, pos.pos.getX(), pos.pos.getY(), pos.pos.getZ());
        ensureContext(pos, () -> this.contextVec.vec.add(pos.contextVec.vec));
    }

    public void sub(LittleAbsoluteVec pos) {
        this.pos = LittleUtils.subtractVectors(this.pos, pos.pos);
    ...
    public void sub(Vector3i vec) {
        pos = LittleUtils.subtractVectors(pos, vec);
    }
    ...
    public double getVanillaGrid(EnumFacingProxy.Axis axis) {
        switch (axis) {
        case X:
            return getPosX();
        case Y:
    ...
    public double getPosX() {
        return pos.getX() + contextVec.vec.x;
    }

    public double getPosY() {
        return pos.getY() + contextVec.vec.y;
    }

    public double getPosZ() {
        return pos.getZ() + contextVec.vec.z;
    }

    public Vector3d getVec3d() {
