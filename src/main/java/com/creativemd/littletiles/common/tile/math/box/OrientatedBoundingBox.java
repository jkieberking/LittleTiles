...
import com.creativemd.littletiles.common.utils.collision.CollidingPlaneProxy;
import com.creativemd.littletiles.common.utils.collision.IntersectionHelperProxy;
...
public class OrientatedBoundingBox extends CreativeAxisAlignedBBProxy {

    public IVecOriginProxy origin;
    public CollidingPlaneProxy.PlaneCache cache;

    public void buildCache() {
        this.cache = new CollidingPlaneProxy.PlaneCache(this);
    }

    public OrientatedBoundingBox(IVecOriginProxy origin, double x1, double y1, double z1, double x2, double y2, double z2) {
        super(x1, y1, z1, x2, y2, z2);
        this.origin = origin;
    }

    public OrientatedBoundingBox(IVecOriginProxy origin, AxisAlignedBB bb) {
...
//    @Override
    public OrientatedBoundingBox offset(Vector3d vec) {
    ...
    public double calculateDistanceRotated(AxisAlignedBB other, Axis axis, double offset) {
        ...
        EnumFacingProxy facing = EnumFacingProxy.getFacingFromAxis(!positive ? EnumFacingProxy.AxisDirection.POSITIVE : EnumFacingProxy.AxisDirection.NEGATIVE, axis);
        ...
        Vector3d[] corners = BoxUtilsProxy.getOuterCorner(facing, origin, this, minOne, minTwo, maxOne, maxTwo);

        Vector3d outerCorner = corners[0];
        double outerCornerOne = VectorUtilsProxy.get(one, outerCorner);
        double outerCornerTwo = VectorUtilsProxy.get(two, outerCorner);
        double outerCornerAxis = VectorUtilsProxy.get(axis, outerCorner);
        ...
        if (outerCornerOffsetOne == 0 && outerCornerOffsetTwo == 0) {
            ...
            if (positive)
                return VectorUtilsProxy.get(axis, outerCorner) - closestValue;
            return closestValue - VectorUtilsProxy.get(axis, outerCorner);
        }
        ...
        directions[0] = new Vector2d(VectorUtilsProxy.get(one, corners[1]) - outerCornerOne, VectorUtilsProxy.get(two, corners[1]) - outerCornerTwo);
        directions[1] = new Vector2d(VectorUtilsProxy.get(one, corners[2]) - outerCornerOne, VectorUtilsProxy.get(two, corners[2]) - outerCornerTwo);
        directions[2] = new Vector2d(VectorUtilsProxy.get(one, corners[3]) - outerCornerOne, VectorUtilsProxy.get(two, corners[3]) - outerCornerTwo);
        ...
        face_loop: for (int i = 0; i < 3; i++) { // Calculate faces
            ...
            double firstAxisValue = VectorUtilsProxy.get(axis, corners[indexFirst + 1]);
            double secondAxisValue = VectorUtilsProxy.get(axis, corners[indexSecond + 1]);
            ...
            if (allInside) {
                ...
                List<Vector2d> points = IntersectionHelperProxy.getIntersectionShape(vectorsRelative);
