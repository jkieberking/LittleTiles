...
import com.creativemd.littletiles.common.utils.math.*;
...
//        boolean[][] cache = new boolean[Rotation.values().length][EnumFacingProxy.values().length];
//        AlignedBox box = new AlignedBox();
//        Vector3f center = new Vector3f(0.5F, 0.5F, 0.5F);
//        for (int i = 0; i < cache.length; i++) {
//            Rotation rotation = Rotation.values()[i];
//            boolean[] flipped = cache[i];
//            for (int j = 0; j < EnumFacingProxy.values().length; j++) {
//                EnumFacingProxy facing = EnumFacingProxy.values()[j];
//                BoxFace face = BoxFace.get(facing);
//                BoxCornerProxy corner = face.getCornerInQuestion(false, false);
//
//                Vector3f vec = box.getCorner(corner);
//                vec.sub(center);
//                RotationUtilsProxy.rotate(vec, rotation);
//                vec.add(center);
//
//                EnumFacingProxy rotatedFacing = RotationUtilsProxy.rotate(facing, rotation);
...
//        boolean[][] cache = new boolean[Axis.values().length][EnumFacingProxy.values().length];
//        AlignedBox box = new AlignedBox();
//        Vector3f center = new Vector3f(0.5F, 0.5F, 0.5F);
//        for (int i = 0; i < cache.length; i++) {
...
//                Vector3f vec = box.getCorner(corner);
//                vec.sub(center);
//                RotationUtilsProxy.flip(vec, axis);
//                vec.add(center);
//
//                EnumFacingProxy rotatedFacing = RotationUtilsProxy.flip(facing, axis);
//                BoxFace rotatedFace = BoxFace.get(rotatedFacing);
//
//                if (vec.epsilonEquals(box.getCorner(rotatedFace.getCornerInQuestion(false, false)), 0.0001F) || vec
//                        .epsilonEquals(box.getCorner(rotatedFace.getCornerInQuestion(true, false)), 0.0001F))
...
//
    public LittleTransformableBox(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, LittleSlice slice) {
        ...
        Axis one = EnumFacingProxy.Axis.getOne(slice.axis);
        Axis two = EnumFacingProxy.Axis.getTwo(slice.axis);

        EnumFacingProxy preferedSide = slice.getPreferedSide(size);
        CornerCache corners = new CornerCache(true);

        BoxCornerProxy emptyCorner1 = slice.getEmptyCorner();
        BoxCornerProxy emptyCorner2 = emptyCorner1.flip(slice.axis);

        EnumFacingProxy.Axis axis = preferedSide.getAxis();
    ...
    public LittleTransformableBox(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, LittleSlice slice, float startOne, float startTwo, float endOne, float endTwo) {
        ...
        EnumFacingProxy.Axis one = EnumFacingProxy.Axis.getOne(slice.axis);
        EnumFacingProxy.Axis two = EnumFacingProxy.Axis.getTwo(slice.axis);

        Vector3d size = new Vector3d(maxX - minX, maxY - minY, maxZ - minZ);
        VectorUtilsProxy.set(size, Math.abs(startOne - endOne), one);
        VectorUtilsProxy.set(size, Math.abs(startTwo - endTwo), two);

        EnumFacingProxy preferedSide = EnumFacingProxy.fromEnumFacing(slice.getPreferedSide(size));
        ...
        Ray2dProxy ray = new Ray2dProxy(one, two, minOne + endOne, minTwo + endTwo, minOne + startOne, minTwo + startTwo);
        ...
        EnumFacingProxy.Axis axis = preferedSide.getAxis();
        int newValue = preferedSide.getAxisDirection() == EnumFacingProxy.AxisDirection.POSITIVE ? (axis == EnumFacingProxy.Axis.getOne(one) ? newMinOne : newMinTwo) : (axis == EnumFacingProxy.Axis.getOne(one) ? newMaxOne : newMaxTwo);
    ...
    public void setFlipped(int facing, boolean value) {
        data[0] = IntegerUtilsProxy.set(getIndicator(), flipStartIndex + facing, value);
        changed();
    }

    public boolean getFlipped(int facing) {
        return IntegerUtilsProxy.bitIs(getIndicator(), flipStartIndex + facing);
    }

    public void setFlipped(EnumFacingProxy facing, boolean value) {
        data[0] = IntegerUtilsProxy.set(getIndicator(), flipStartIndex + facing.ordinal(), value);
        changed();
    }

    public boolean getFlipped(EnumFacingProxy facing) {
        return IntegerUtilsProxy.bitIs(getIndicator(), flipStartIndex + facing.ordinal());
    }
    ...
    public Vector3f[] getTiltedCorners() {
        Vector3f[] corners = new Vector3f[BoxCornerProxy.values().length];
        ...
        for (int i = 0; i < corners.length; i++) {
            ...
            int z = 0;
            int index = i * 3;
            if (IntegerUtilsProxy.bitIs(indicator, index)) {
                x = getData(activeBits);
                activeBits++;
            }

            if (IntegerUtilsProxy.bitIs(indicator, index + 1)) {
                y = getData(activeBits);
                activeBits++;
            }

            if (IntegerUtilsProxy.bitIs(indicator, index + 2)) {
    ...
    private static boolean checkEqual(Vector3f[] corners, LittleVec[] otherCorners, BoxCornerProxy[] toCheck, EnumFacingProxy.Axis axis) {
        for (int j = 0; j < toCheck.length; j++) {
            BoxCornerProxy corner = toCheck[j];
    ...
    private static VectorFan createStrip(BoxCornerProxy[] corners, Vector3f[] vec) {
        Vector3f[] coords = new Vector3f[corners.length];
        int index = 0;
        outer: for (int i = 0; i < coords.length; i++) {
            for (int j = 0; j < index; j++)
                if (LittleUtils.epsilonEquals(vec[corners[i].ordinal()], coords[j], VectorFan.EPSILON))
                    continue outer;
    ...
    public synchronized VectorFanCache requestCache() {
        ...
        NormalPlaneProxy[] planes = new NormalPlaneProxy[EnumFacingProxy.values().length];
        for (int i = 0; i < planes.length; i++) {
            EnumFacingProxy facing = EnumFacingProxy.values()[i];
            EnumFacingProxy.Axis axis = facing.getAxis();

            NormalPlaneProxy plane = new NormalPlaneProxy(new Vector3f(), new Vector3f());
            plane.origin.set(0, 0, 0);
            VectorUtilsProxy.set(plane.origin, get(facing), axis);

            plane.normal.set(0, 0, 0);
            VectorUtilsProxy.set(plane.normal, facing.getAxisDirection().getOffset(), axis);
        ...
        NormalPlaneProxy[] tiltedPlanes = new NormalPlaneProxy[EnumFacingProxy.values().length * 2]; // Stores all tilted planes to use them for cutting later
        ...
        for (int i = 0; i < EnumFacingProxy.values().length; i++) {
            ...
            BoxFaceProxy face = BoxFaceProxy.get(facing);
            boolean inverted = getFlipped(facing);

            VectorFanFaceCache faceCache = cache.faces[i];

            BoxCornerProxy[] first = face.getTriangleFirst(inverted);
            Vector3f firstNormal = BoxFaceProxy.getTraingleNormal(first, corners);
            boolean firstSame = checkEqual(corners, boxCorners, first, facing.getAxis());

            BoxCornerProxy[] second = face.getTriangleSecond(inverted);
            Vector3f secondNormal = BoxFaceProxy.getTraingleNormal(second, corners);
            ...
            if (parallel) {
                if (!firstSame && !LittleUtils.epsilonEquals(firstNormal, ZERO, VectorFan.EPSILON)) {
                    faceCache.tiltedStrip1 = createStrip(face.corners, corners);
                    if (faceCache.tiltedStrip1 != null)
                        tiltedPlanes[i * 2] = new NormalPlaneProxy(corners[first[0].ordinal()], firstNormal);
                }
            } else {
                if (!firstSame && !LittleUtils.epsilonEquals(firstNormal, ZERO, VectorFan.EPSILON)) {
                ...
                if (!secondSame && !LittleUtils.epsilonEquals(secondNormal, ZERO, VectorFan.EPSILON)) {
                    faceCache.tiltedStrip2 = createStrip(second, corners);
                    if (faceCache.tiltedStrip2 != null)
                        tiltedPlanes[i * 2 + 1] = new NormalPlaneProxy(corners[second[0].ordinal()], secondNormal);
                }
            }
        ...
        for (int i = 0; i < EnumFacingProxy.values().length; i++) {
            ...
            NormalPlaneProxy plane = planes[i];

            for (int j = 0; j < EnumFacingProxy.values().length; j++) {
                VectorFanFaceCache faceCache = cache.faces[j];
                if (faceCache.tiltedStrip1 == null && faceCache.tiltedStrip2 == null) {
                    NormalPlaneProxy cutPlane1 = tiltedPlanes[j * 2];
                    NormalPlaneProxy cutPlane2 = tiltedPlanes[j * 2 + 1];
                    if (faceCache.convex) {
                        if (cutPlane1 != null)
                    ...
                    NormalPlaneProxy cutPlane1 = null;
                    NormalPlaneProxy cutPlane2 = null;
    ...
    public LittleBox combineBoxes(LittleBox box) {
        if (box instanceof LittleTransformableBox) {
            EnumFacingProxy facing = box.sharedBoxFaceWithoutBounds(this);
            ...
            Axis one = RotationUtilsProxy.getOne(facing.getAxis());
            Axis two = RotationUtilsProxy.getTwo(facing.getAxis());
            ...
            BoxCornerProxy[] corners = BoxCornerProxy.faceCorners(facing);
            for (int i = 0; i < corners.length; i++) {
                BoxCornerProxy corner = corners[i];
                BoxCornerProxy otherCorner = corner.flip(facing.getAxis());
                ...
                if (ray.direction.x == 0 && ray.direction.y == 0 && ray.direction.z == 0) {
                    BoxCornerProxy newCorner = otherCorner.flip(one);
    ...
    protected boolean intersectsWith(VectorFanCache cache) {
            ...
            if (face.hasAxisStrip() && otherFace.hasAxisStrip()) {
                ...
                for (VectorFan fan : face.axisStrips)
                    for (VectorFan fan2 : otherFace.axisStrips)
                        if (VectorUtilsProxy.get(axis, fan.get(0)) == VectorUtilsProxy.get(axis, fan2.get(0)) && fan
                                .intersect2d(fan2, one, two, facing.getAxisDirection() == EnumFacingProxy.AxisDirection.POSITIVE))
                            return true;
            }
        }

        List<List<NormalPlaneProxy>> shapes = new ArrayList<>();
        ...
        for (int i = 0; i < ownCache.faces.length; i++) {
            ...
            if (face.hasTiltedStrip()) {
                NormalPlaneProxy plane1 = face.tiltedStrip1 != null ? face.tiltedStrip1.createPlane() : null;
                NormalPlaneProxy plane2 = face.tiltedStrip2 != null ? face.tiltedStrip2.createPlane() : null;
                ...
                if (face.convex) {
                    ...
                    for (int j = 0; j < sizeBefore; j++) {
                        if (plane1 != null && plane2 != null) {
                            List<NormalPlaneProxy>newList = new ArrayList<>(shapes.get(j));
                            shapes.get(j).add(plane1);
                            newList.add(plane2);
                            shapes.add(newList);
                        } else if (plane1 != null)
    ...
    @Override
    public void rotateBox(RotationProxy rotation, LittleVec doubledCenter) {
        ...
        for (int i = 0; i < EnumFacingProxy.values().length; i++) {
            RotationProxy facing = RotationUtilsProxy.rotate(EnumFacingProxy.getFront(i), rotation);
    ...
    public void flipBox(Axis axis, LittleVec doubledCenter) {
        ...
        for (int i = 0; i < EnumFacingProxy.values().length; i++) {
            EnumFacingProxy facing = RotationUtilsProxy.flip(EnumFacingProxy.getFront(i), axis);
...
//            volume.addBox(minX, minY, minZ, maxX, maxY, maxZ);
    ...
    @Override
    public LittleBox grow(EnumFacingProxy facing) {
    ...
    @Override
    public LittleBox shrink(EnumFacingProxy facing, boolean toLimit) {
    ...
    protected Iterator<TransformableVec> corners() {
        return new Iterator<TransformableVec>() {
            ...
            void findNext() {
                ...
                while (corner < BoxCornerProxy.values().length) {
            ...
            public TransformableVec next() {
                ...
                TransformableVec toReturn = holder.set(activeBits, BoxCornerProxy.values()[corner], vec.x, vec.y, vec.z);
                findNext();
                if (!hasNext())
                    holder = null;
                return toReturn;
    ...
    protected Iterator<TransformablePoint> points() {
        return new Iterator<TransformablePoint>() {
            ...
            void findNext() {
                inc();
                while (index < flipStartIndex && !IntegerUtilsProxy.bitIs(indicator, index))
                    inc();
            ...
            public TransformablePoint next() {
                if (point == null)
                    throw new NoSuchElementException();
                TransformablePoint toReturn = point.set(corner * 3 + axisIndex, activeBits, Axis.values()[axisIndex], BoxCornerProxy.values()[corner]);
                findNext();
    ...
    @Override
    public MovingObjectPosition calculateIntercept(LittleGridContext context, BlockPos pos, Vector3d vecA, Vector3d vecB) {
        ...
        LittleUtils.scaleInPlace(start, context.size);
        LittleUtils.scaleInPlace(end, context.size);

        Ray3fProxy ray = new Ray3fProxy(start, end);

        BlockPos blockPos = LittleUtils.subtractVectors(vecA, pos.getX(), pos.getY(), pos.getZ());
        vecA = new Vector3d(blockPos.x, blockPos.y, blockPos.z);

        Vector3d collision = null;
        EnumFacingProxy collided = null;

        for (int i = 0; i < EnumFacingProxy.values().length; i++) {
            VectorFanFaceCache face = cache.get(EnumFacingProxy.values()[i]);
            for (VectorFan strip : face) {
                Vector3d temp = strip.calculateIntercept(ray);
                if (temp != null)
                    temp = LittleUtils.scaleNotInPlace(temp, context.pixelSize);
                if (temp != null && isClosest(vecA, collision, temp)) {
                    collided = EnumFacingProxy.values()[i];
                    collision = temp;
                }
            }
        }
        ...
        BlockPos finalPosition = LittleUtils.addVectors(collision, pos.getX(), pos.getY(), pos.getZ());
        return new MovingObjectPosition(finalPosition.x, finalPosition.y, finalPosition.z, collided.getIndex() /* @TODO not sure if this will work for side hit */, Vec3.createVectorHelper(collision.x, collision.y, collision.z) /* @TODO this might be the wrong one? */);
    }
    ...
    class TransformableVec {
        ...
        public BoxCornerProxy getCorner() {
            return corner;
        }
        ...
        public TransformableVec set(int index, BoxCornerProxy corner, int x, int y, int z) {
            this.x = x;
            this.y = y;
            this.z = z;
            this.index = index;
    ...
    class TransformablePoint {
        ...
        public TransformablePoint set(int progressIndex, int index, Axis axis, BoxCornerProxy corner) {
    ...
    public class CornerCache {
        ...
        public LittleVec[] corners = new LittleVec[BoxCornerProxy.values().length];

        public LittleVec getOrCreate(BoxCornerProxy corner) {
        ...
        public void setAbsolute(BoxCornerProxy corner, LittleVec vec) {
            corners[corner.ordinal()] = vec;
            if (relative) {
        ...
        public void setAbsolute(BoxCornerProxy corner, EnumFacingProxy.Axis axis, int value) {
            if (relative)
        ...
        public void setRelative(BoxCornerProxy corner, LittleVec vec) {
            corners[corner.ordinal()] = vec;
            if (!relative) {
                vec.x += get(corner, Axis.X);
                vec.y += get(corner, Axis.Y);
    ...
    public void setRelative(BoxCornerProxy corner, EnumFacingProxy.Axis axis, int value) {
        if (relative)
            getOrCreate(corner).set(axis , value);
        else
            getOrCreate(corner).set(axis, value + get(corner, axis));
        ...
        public int[] getData() {
            ...
            for (int i = 0; i < corners.length; i++) {
                ...
                if (relative) {
                    if (vec.x != 0) {
                        indicator = IntegerUtilsProxy.set(indicator, index);
                        data.add(vec.x);
                    }
                    if (vec.y != 0) {
                        indicator = IntegerUtilsProxy.set(indicator, index + 1);
                        data.add(vec.y);
                    }
                    if (vec.z != 0) {
                        indicator = IntegerUtilsProxy.set(indicator, index + 2);
                        data.add(vec.z);
                    }
                } else {
                    BoxCornerProxy corner = BoxCornerProxy.values()[i];
...
//
    ...
    public VectorFanFaceCache get(EnumFacingProxy facing) {
        ...
        protected boolean intersects(NormalPlaneProxy plane1, NormalPlaneProxy plane2) {
            for (int i = 0; i < faces.length; i++)
        ...
        public boolean isInside(List<NormalPlaneProxy>shape, Vector3f vec) {
    ...
    public static class VectorFanFaceCache implements Iterable<VectorFan> {
        ...
        public void collectAllTiltedStripsRendering(List<VectorFan> fans) {
            if (hasSingleTiltedStripRendering())
                ...
                fans.addAll((List<? extends VectorFan>) stripsSorted);
        }

        public void sortTiltedStrips(VectorFanCache cache, NormalPlaneProxy plane1, NormalPlaneProxy plane2) {
        ...
        private void sortTiltedStrip(VectorFan fan, VectorFanCache cache, NormalPlaneProxy plane) {
        ...
        public static EnumFacingProxy nearest(Vector3f vec) {
            return nearest(vec.x, vec.y, vec.z);
        }

        public static EnumFacingProxy nearest(float x, float y, float z) {
            if (x == 0 && y == 0 && z == 0)
                return null;

            EnumFacingProxy facing = null;
            float distance = Float.MIN_VALUE;

            for (int i = 0; i < EnumFacingProxy.values().length; i++) {
                EnumFacingProxy f = EnumFacingProxy.values()[i];
                float newDistance = x * f.getDirectionVec().x() + y * f.getDirectionVec().y() + z * f.getDirectionVec().z();
        ...
        public void cutAxisStrip(EnumFacingProxy facing, NormalPlaneProxy plane, NormalPlaneProxy plane2) {
            Axis axis = facing.getAxis();
            Axis one = RotationUtilsProxy.getOne(axis);
            Axis two = RotationUtilsProxy.getTwo(axis);
            boolean inverse = facing.getAxisDirection() == EnumFacingProxy.AxisDirection.POSITIVE;
        ...
        public void cutAxisStrip(NormalPlaneProxy plane) {
            int i = 0;
        ...
        public boolean isInside(List<List<NormalPlaneProxy>> shapes, List<CenterPoint> centers) {
            if (!convex) {
        ...
        public Iterable<VectorFan> tiltedSorted() {
            ...
            if (hasSingleTiltedStripRendering())
                return new SingletonListProxy<>((VectorFan) stripsSorted);
