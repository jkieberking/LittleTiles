...
//
    public LittleTransformableBox(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, LittleSlice slice) {
        ...
        Axis one = RotationUtilProxy.getOne(slice.axis);
        Axis two = RotationUtilProxy.getTwo(slice.axis);
    ...
    public LittleTransformableBox(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, LittleSlice slice, float startOne, float startTwo, float endOne, float endTwo) {
        ...
        EnumFacingProxy.Axis axis = preferedSide.getAxis();
        int newValue = preferedSide.getAxisDirection() == EnumFacingProxy.AxisDirection.POSITIVE ? (axis == RotationUtilProxy.getOneEnumFacing(one) ? newMinOne : newMinTwo) : (axis == RotationUtilProxy.getOneEnumFacing(one) ? newMaxOne : newMaxTwo);
...
//
    public class CornerCache {
        ...
        public void setAbsolute(BoxCorner corner, LittleVec vec) {
            corners[corner.ordinal()] = vec;
            if (relative) {
                vec.x -= get(corner, Axis.Xaxis);
                vec.y -= get(corner, Axis.Yaxis);
                vec.z -= get(corner, Axis.Zaxis);
            }
        }

        public void setAbsolute(BoxCorner corner, EnumFacingProxy.Axis axis, int value) {
            if (relative)
    ...
    public void setRelative(BoxCorner corner, EnumFacingProxy.Axis axis, int value) {
        if (relative)
            getOrCreate(corner).set(axis , value);
        else
            getOrCreate(corner).set(axis, value + get(corner, axis));
        ...
        public int[] getData() {
            ...
            for (int i = 0; i < corners.length; i++) {
                ...
                if (relative) {
                    ...
                    BoxCorner corner = BoxCorner.values()[i];
                    if (vec.x != get(corner, Axis.Xaxis)) {
                        indicator = IntegerUtilsProxy.set(indicator, index);
                        data.add(vec.x - get(corner, Axis.Xaxis));
                    }
                    if (vec.y != get(corner, Axis.Yaxis)) {
                        indicator = IntegerUtilsProxy.set(indicator, index + 1);
                        data.add(vec.y - get(corner, Axis.Yaxis));
                    }
                    if (vec.z != get(corner, Axis.Zaxis)) {
                        indicator = IntegerUtilsProxy.set(indicator, index + 2);
                        data.add(vec.z - get(corner, Axis.Zaxis));
                    }
                }
            }
