...
//
    ...
    public boolean execute() {
        ...
        // @TODO implement action executing
        try {
            if (action(player)) {
                rememberAction(this);
                MinecraftForge.EVENT_BUS.post(new ActionEvent(this, ActionEvent.ActionType.normal, player));
...
//
    ...
    public static void handleExceptionClient(LittleActionException e) {
        ...
        if (message != null)
            ...
            Minecraft.getMinecraft().thePlayer.sendChatMessage(e.getLocalizedMessage());
        // @TODO send status
//            Minecraft.getMinecraft().thePlayer.sendStatusMessage(new TextComponentString(e.getLocalizedMessage()), true);
    }
//
...
//
    public static boolean canPlaceInside(LittlePreviews previews, World world, BlockPos pos, boolean placeInside) {
        ...
        if (block.isReplaceable(world, pos.x(), pos.y(), pos.z()) /* || block instanceof BlockTilt */) {
            if (!placeInside) {
    ...
    public static TileEntityLittleTilesProxy loadTe(EntityPlayer player, World world, BlockPos pos, MutableInt affected, boolean shouldConvert, int attribute) throws LittleActionException {
...
////            if (tileEntity == null && shouldConvert) {
...
//                TileEntityLittleTilesProxy te = (TileEntityLittleTilesProxy) world.getTileEntity(pos.x, pos.y, pos.z);
...
//    }
//
    public static boolean isAllowedToInteract(EntityPlayer player, boolean rightClick) {
        // @TODO do we need to restrict them?
//        if ((!rightClick && !player.capabilities.allowEdit))
//            return false;
...
//                return false;
        } /*else if (!rightClick && !player.canPlayerEdit(pos.x, pos.y, pos.z, stack.i))
            return false;*/
